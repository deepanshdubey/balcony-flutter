...
class OngoingFormPage extends StatefulWidget {
  const OngoingFormPage({Key? key}) : super(key: key);
...
class _OngoingFormPageState extends State<OngoingFormPage> {
  ...
  void _submitForm() {
    if (_formKey.currentState!.validate() && _selectedTenant != null) {
      ...
      conciergeStore.ongoingAdd(formData);
  ...
  @override
  void initState() {conciergeStore.conciergeTenantAll();
    addDisposer();
    super.initState();
  }
  ...
  void addDisposer() {
    disposers ??= [
      ...
      reaction((_) => conciergeStore.ongoingAddResponse, (response) {
                            ...
                            builder: (context) {
                              return Container(
                                ...
                                child: DropdownButtonFormField<String>(
                                  ...
                                  onChanged: (value) {
                                    setState(() {
                                      _selectedTenant = value; // Store the selected property ID
                                      _selectedTenantId = conciergeStore.conciergeTenantAllResponse?.tenants?.conciergeTenants?.firstWhere(
                                            (tenant) => tenant.info?.firstName == value, // Match ID
